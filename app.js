...
function renderCharts() {
  ...
  // Risk chart
  const riskCounts = { 'On Track': 0, 'At Risk': 0 };
  STUDENTS.forEach(s => riskCounts[s.risk] = (riskCounts[s.risk] || 0) + 1);
  new Chart(document.getElementById('riskChart'), {
    type: 'doughnut',
    data: {
      labels: Object.keys(riskCounts),
      datasets: [{ data: Object.values(riskCounts), backgroundColor: ['#22c55e', '#ef4444'] }]
    },
  ...
  new Chart(document.getElementById('statusChart'), {
    ...
    data: {
      ...
      datasets: [{ data: Object.values(statusCounts), backgroundColor: ['#38bdf8', '#f59e0b', '#a78bfa'] }]
    },
    options: { plugins: { legend: { position: 'bottom' } } }
  });
}
